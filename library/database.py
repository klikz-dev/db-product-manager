...
class DatabaseManager:
    ...
    def statusSync(self, fullSync=False):
        ...
        self.csr.execute(f"""
                    SELECT P.ProductID, P.ManufacturerPartNumber, P.SKU, P.Published
                    FROM Product P
        ...
        for row in rows:
            ...
            try:
                ...
                products = self.Feed.objects.filter(mpn=mpn)
    ...
    def downloadFileFromSFTP(self, src, dst, fileSrc=True, delete=True):
        host = const.sftp[self.brand]["host"]
        port = const.sftp[self.brand]["port"]
