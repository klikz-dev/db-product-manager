...
class Command(BaseCommand):
    ...
    def pillowSample(self):
        ...
        pillows = Mindthegap.objects.filter(ptype="Pillow")
        for pillow in pillows:
            try:
                fabric = Mindthegap.objects.get(
                ...
                debug("Mindthegap", 1, "Matching Fabric not found for pillow Pattern: {} and Color: {}".format(
            ...
            for metafield in data['metafields']:
                if metafield['key'] == "fabric_id":
                    ...
                    if response.status_code == 200:
                        debug("Mindthegap", 0, "Fabric link has been updated successfully. Pillow: {}, Fabric: {}".format(
                            productId, fabric.productId))
                    else:
                        debug("Mindthegap", 1, "Metafield Update API error. {}".format(
                            response.text))
            ...
            if not isFabricLinked:
                ...
                if response.status_code == 201:
                    debug("Mindthegap", 0, "Fabric has been linked successfully. Pillow: {}, Fabric: {}".format(
                        productId, fabric.productId))
                else:
                    debug("Mindthegap", 1, "Metafield Create API error. {}".format(
                        response.text))
